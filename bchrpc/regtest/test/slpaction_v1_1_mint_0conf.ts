...
describe("SlpAction: Mint V1 (unconfirmed)", () => {
  ...
  step("SlpAction: SLP_V1_MINT", async () => {
    ...
    assert.ok(info.getValidityJudgement() === SlpTransactionInfo.ValidityJudgement.VALID);
    assert.ok(info.getSlpAction() === SlpAction.SLP_V1_MINT);
    ...
    assert.ok(baton.getTokenType() === 1);
    assert.ok(baton.getSlpAction() === SlpAction.SLP_V1_GENESIS);  // the slp action from previous txn
    ...
    assert.ok(outputs[1].getSlpToken()!.getAddress() === wallet3.slpRegTestAddressNoPrefix);
    assert.ok(outputs[1].getSlpToken()!.getTokenType() === 1);
    assert.ok(outputs[1].getSlpToken()!.getSlpAction() === SlpAction.SLP_V1_MINT);
    ...
    assert.ok(outputs[2].getSlpToken()!.getAddress() === wallet3.slpRegTestAddressNoPrefix);
    assert.ok(outputs[2].getSlpToken()!.getTokenType() === 1);
    assert.ok(outputs[2].getSlpToken()!.getSlpAction() === SlpAction.SLP_V1_MINT);
