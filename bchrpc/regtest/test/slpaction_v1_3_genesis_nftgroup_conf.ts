...
describe("SlpAction: Genesis NFT1 Group (unconfirmed)", () => {
  ...
  step("send to wallet2", async () => {
    ...
    let resTx = await bchd1Grpc.getTransaction({ hash: prevOutBch.txid, reversedHashOrder: true, includeTokenMetadata: true });
    while (resTx.getTransaction()!.getConfirmations() < 1) {
      await sleep(500);
      resTx = await bchd1Grpc.getTransaction({ hash: prevOutBch.txid, reversedHashOrder: true, includeTokenMetadata: true });
    }
  ...
  step("SlpAction: SLP_NFT1_GROUP_GENESIS (without baton)", async () => {
    ...
    const genesisMintAmount = new mdm.BN(1337);
    tokenMetadata = { name: "type 1 test", ticker: "t1t", decimals: 9, url: "test.com", hashHex: "" };
    const slpGenesisOpReturn = mdm.NFT1.Group.genesis(tokenMetadata.ticker, tokenMetadata.name, tokenMetadata.url, tokenMetadata.hashHex, tokenMetadata.decimals, null, genesisMintAmount);
    txn.addOutput(new Transaction.Output({
      script: slpGenesisOpReturn,
      satoshis: 0,
    }));
